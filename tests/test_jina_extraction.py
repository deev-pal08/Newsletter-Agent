...
def test_parse_markdown_heading_links() -> None:
    articles = _parse_markdown_articles(SAMPLE_MARKDOWN, "Test", "https://example.com")
...
def test_parse_markdown_standalone_links() -> None:
    articles = _parse_markdown_articles(SAMPLE_MARKDOWN_LINKS, "Test", "https://example.com")
    assert len(articles) == 3
...
def test_parse_markdown_too_few_articles() -> None:
    articles = _parse_markdown_articles(
        "# One\n[Short](http://a.com)", "Test", "https://x.com",
    )
...
async def test_try_jina_success() -> None:
    ...
    async with httpx.AsyncClient() as http:
        articles = await _try_jina(http, "Test Blog", "https://example.com/blog")
    assert len(articles) == 3
...
async def test_try_jina_too_short() -> None:
    ...
    async with httpx.AsyncClient() as http:
        articles = await _try_jina(http, "Test", "https://example.com/blog")
    assert len(articles) == 0
