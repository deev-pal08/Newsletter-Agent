...
Every `send` run searches the web for fresh articles via the Tavily Search API. Queries are generated by DeepSeek V4 Flash based on your `AboutMe.md` profile and configured interests — each query targets a different interest area for diverse coverage (5 queries for normal runs, 10 for topic-focused runs).

In topic mode (`--topic`), all 10 queries target different angles of the topic: breaking news, research papers, tools, defenses, bug bounty reports, conference talks, standards, tutorials, CVEs, and expert analysis.
...
2. **Web article search** — LLM-generated Tavily queries targeting each interest area (5 queries normal, 10 for topic mode)
